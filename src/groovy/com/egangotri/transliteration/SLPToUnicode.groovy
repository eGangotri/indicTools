...
class SLPToUnicode {
    ...
    public static getHalant(String a_akshar) {
        IndicLanguagesChart.toUTF(a_akshar, 72)
    }
    ...
    public static String _transform(List<Hashtable<String, String>> script, String a_symbol, text) {
        if (script == null) {
            script = IndicLanguagesChart.populateHashTable(a_symbol)
        }
        IndicLanguagesChart.transform(script[0], script[1], getHalant(a_symbol), text)
    }
